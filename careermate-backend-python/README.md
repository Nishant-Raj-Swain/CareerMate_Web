...
## Sending contact-form messages to your email

Every message submitted through the site's Contact form is saved to the
database *and* emailed to `hello@careermate.app` (or whatever you set
`CONTACT_RECEIVER_EMAIL` to). To turn on the email side, fill in these
vars in `.env`:
...
CONTACT_RECEIVER_EMAIL=hello@careermate.app
```

**Using Gmail:** you can't use your normal Gmail password here — Google
requires an [App Password](https://myaccount.google.com/apppasswords)
(needs 2-Step Verification turned on first). Generate one and use it as
`SMTP_PASS`.

**Using a transactional email service instead (recommended for
production):** any provider that gives you SMTP credentials works the same
way — e.g. SendGrid, Mailgun, Postmark, Resend, Amazon SES. Just swap
`SMTP_HOST`/`SMTP_USER`/`SMTP_PASS` for the values they give you; nothing
else in the code changes.

**If `SMTP_HOST` is left blank**, sending is skipped — the message is still
saved to the database, and the API still returns success. Check the server
logs for a `[mailer] SMTP_HOST not set` line if you're expecting an email
that didn't arrive.
...
| POST   | `/api/contact`        | `{ name, email, message }`  | Stores a message and emails it     |
